...
def parse_args():
    ...
    p.add_argument('--w_cls', type=float, default=1.5)
    p.add_argument('--w_det', type=float, default=1.5)

    # seg loss settings
    p.add_argument('--seg_pos_weight', type=float, default=8.0)
...
def seg_loss_total(mask_logits, gt_mask, bce, dice_weight=0.5):
    loss_bce = bce(mask_logits, gt_mask)
    loss_dice = dice_loss_from_logits(mask_logits, gt_mask)
    return loss_bce + float(dice_weight) * loss_dice, loss_bce, loss_dice


@torch.no_grad()
def quick_val(model, loader, device, loss_fns, weights, dice_weight):
    bce, ce, l1 = loss_fns
    ...
    for batch in loader:
        ...
        loss_seg, _, _ = seg_loss_total(mask_logits, gt_mask, bce, dice_weight=dice_weight)
...
def main():
    ...
    print('Approx size (fp32): %.2f MB' % (total * 4 / 1024 / 1024))
    print(f'Device: {device}')

    pos_weight = torch.tensor([args.seg_pos_weight], device=device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in pbar:
            ...
            loss_seg, loss_bce, loss_dice = seg_loss_total(mask_logits, gt_mask, bce, dice_weight=args.dice_weight)
            loss_cls = ce(cls_logits, gt_label)
            loss_det = l1(bbox_pred, gt_bbox_norm)
            ...
            pbar.set_postfix(OrderedDict([
                ...
                ('seg_bce', f'{loss_bce.item():.4f}'),
                ('seg_dice', f'{loss_dice.item():.4f}'),
